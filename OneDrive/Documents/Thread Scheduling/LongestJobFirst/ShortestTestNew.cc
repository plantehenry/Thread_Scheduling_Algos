...
int recursiveCallxTimes(uint32_t times){
    if(times != 0){
        recursiveCallxTimes(times - 1);
    }
    return times -1;
}
...
void kernelMain(void) {
    ...
    for (int i=0; i<600; i++) {
        ...
        thread(numLoops, [numLoops, &counter, &dummyCounter, &waitingFor, initialCount] {
    ...
    uint32_t average = counter.get() / 8;
    Debug::printf("*** %d\n", average);
}
