...
class MinHeap
{
	heapEntry<T> **harr; // pointer to array of elements in heap
...
public:
	...
	MinHeap(int cap){
		heap_size = 0;
		capacity = cap;
		harr = new heapEntry<T>*[cap];
		lock = new InterruptSafeLock();
		for(int i = 0; i < cap; i++){
			harr[i] = new heapEntry<T>();
		}
	}
	...
	TCB* extractMin(){
		...
		{
			heap_size--;
			TCB* pointer = (TCB*)harr[0];
			return pointer;
			T* returnVal = harr[0]->tcb;
			heapEntry<T>* toDelete = harr[0];
			Debug::printf("extract before call\n");
			delete toDelete;
			Debug::printf("extract after call\n");
			// lock->unlock();
			// Debug::printf("exit %d\n", heap_size);
		...
		heapEntry<T>* root = harr[0];
		harr[0] = harr[heap_size-1];
		//Debug::printf("heapsize -1 %d",* (harr[heap_size - 1]->tcb));
		heap_size--;
		MinHeapify(0);
		T* returnVal = root->tcb;;
		heapEntry<T>* toDelete = root;
		Debug::printf("extract before call %x\n", root);
		delete toDelete;
		Debug::printf("extract after call\n");
		// lock->unlock();
		Debug::printf("exit %d\n", heap_size);
	...
	// Inserts a new key 'k'
	void insertKey(T* k, int predictedTime){
		LockGuard g{*lock};
		...
		heap_size++;
		int i = heap_size - 1;
		Debug::printf("insert before call\n");
		heapEntry<T>* newEntry = new heapEntry<T>();
		Debug::printf("insert after call\n");
		newEntry->predTime = predictedTime;
		newEntry->tcb = k;
		harr[i] = newEntry;
