...
extern "C" void apitHandler(uint32_t* things) {
    ...
    //Debug::printf("*** priorities %d %d %d %d\n", active[1]->priority, active[2]->priority, active[3]->priority, active[0]->priority);
    ...
    SMP::eoi_reg.set(0);
    // potentially dereferncing nullptrs
    if(!active[SMP::me()]->idleOrDummy){
        if(active[SMP::me()] == nullptr || active[SMP::me()]->preemptable){
            if(active[SMP::me()] == nullptr){
                // TCB* fake = fakeThread([]{});
                // active[SMP::me()] = fake;
                // active[SMP::me()]->nullInPit = true;
                active[SMP::me()] = dummy[SMP::me()];
            }
            active[SMP::me()]->cameFromPit = true;
            yield();
        }
    }
}
